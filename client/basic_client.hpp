...
class BasicClient {
  ...
  using Protocol_t = icon::details::Protocol<
      Raw_t, RawBuffer_t,
      icon::details::DataLayout<icon::details::fields::Header,
...
private:
  awaitable<void> init_connection_async() {
    const auto response = co_await async_send_with_response<InternalResponse_t>(
  ...
  awaitable<void> async_do_send(core::Header &&header, Message &&message) {
    ...
    co_await zmq_send_op.async_send(
        build_raw_buffer(Serializable_t<core::Header>{std::move(header)},
                         Serializable_t<Message>{std::move(message)}));
  }

  template <class Response> awaitable<Response> async_receive() {
