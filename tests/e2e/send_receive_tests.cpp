...
TEST_CASE("Endpoint receives and sends messages")
{
  ...
  SECTION("received message contains valid data")
  {
    ...
    auto endpoint = make_endpoint<icon::dummy::TestSeqReq>(bctx, zctx, [&bctx](icon::MessageContext<icon::dummy::TestSeqReq>& context) -> awaitable<void> {
  ...
  SECTION("endpoint responds with a message ")
  {
    ...
    auto endpoint = make_endpoint<icon::dummy::TestSeqReq>(bctx, zctx, [](icon::MessageContext<icon::dummy::TestSeqReq>& context) -> awaitable<void> {
      auto& msg = context.message();
    ...
    co_spawn(bctx, endpoint->run(), detached);
    co_spawn(
      bctx, [&client, &bctx]() -> awaitable<void> {
